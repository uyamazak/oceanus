        ...
        if matchs:
            ...
            for index, org_url in enumerate(matchs):
                ...
                url_obj = Url(url=org_url,
                              ...
                              author=request.user)
                url_obj.save()
                converted_article = converted_article.replace(org_url,
                                                              settings.SITE_URL + "/" + short_id)
...
def index(request):
    """dummy page"""
    logging.info("dummy page access")
...
def _send_beacon(beacon_url):
    try:
        ...
        logging.critical("_send_beacon urlopen error:{}".format(e))
    else:
        logging.info(r.read())


def _create_beacon_url(url_obj, request, provisional_sid):
    endpoint = url_obj.beacon.endpoint
    parameters = {
        "url": url_obj.url,
        "sid": request.COOKIES.get(settings.COOKIE_KEY, provisional_sid),
        "oid": "2",
        "tit": "",
        "evt": "redirect",
...
def _set_oceanus_sid(request, response, provisional_sid):
    if request.COOKIES.get(settings.COOKIE_KEY):
        return response

    max_age = 365 * 24 * 60 * 60 * 10
    response.set_cookie(settings.COOKIE_KEY,
                        value=provisional_sid,
                        max_age=max_age,
...
def redirect_original(request, short_id):
    ...
    provisional_sid = _create_sid()
    beacon_url = _create_beacon_url(url_obj, request,  provisional_sid)
    _send_beacon(beacon_url)
    response = HttpResponseRedirect(url_obj.url)
    response = _set_oceanus_sid(request, response,  provisional_sid)
    return response
