...
class redis2bq():
    ...
    def create_table_name(self, delta_days=0):
        if delta_days != 0:
            ...
            return TABLE_PREFIX + date_delta.strftime('_%Y%m%d')
        else:
            return TABLE_PREFIX + datetime.datetime.now().strftime('_%Y%m%d')
        ...
        while True:
            ...
            while len(lines) < CHUNK_NUM:
                ...
                try:
                    ...
                    logger.info("CHUNK:{0}/{1}".format(len(lines) + 1,
                                                       CHUNK_NUM))
