...
class GazeResource(ExecutionResource):
    method_label = "gaze"
    ...
    def on_get(self, req, resp, site_name=None):
        ...
        item_dict = {
            ...
            # oceanus id
            'oid': (req.get_param('oid', required=False, default="2"),
                    {'type': 'string',
                     'maxlength': 16}
                    ),
        ...
        evt = user_data["evt"]
        redirect_params = parse_qs(req.env.get("QUERY_STRING"))
        path = req.env.get("PATH_INFO")
        if evt == "open":
            redirect_params['evt'] = ["see"]
            redirect_url = path + "?" + urlencode(redirect_params, doseq=True)
